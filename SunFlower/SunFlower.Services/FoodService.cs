...
namespace SunFlower.Services
{
    ...
    public class FoodService : IFood
    {
        ...
        public List<Food> StoreGetFoods(int id,string typeName)
        {
            using (OracleConnection conn = DapperHelper.GetConnString())
            {
                string sql = @"select f.id,s.id as sid, s.storenumber, f.foodname,f.filename,f.foodsprice,f.sale,fp.typename from food f join  foodtype fp on  f.foodtypeid=fp.id join Store s on(f.storenumber=s.storenumber) where s.id=:id and fp.typename=:typename order by f.ID desc ";
                var foodList = conn.Query<Food>(sql, new { id = id,typename=typeName });
                return foodList.ToList<Food>();
            }
        }
        ...
        public List<Food> GetFoodByID(int id)
        {
            using (OracleConnection conn = DapperHelper.GetConnString())
            {
                string sql = @"select f.id,f.foodsprice,f.foodnumber,s.storenumber, f.foodname,f.filename,f.foodsprice,f.sale,f.foodsummary,f.foodtypeid from food f join Store s on(f.storenumber=s.storenumber) where f.id=:id order by ID desc";
                var foodList = conn.Query<Food>(sql, new { id = id });
                return foodList.ToList<Food>();
            }
        }
        ...
        public List<Food> GetFood(int id)
        {
            using (OracleConnection conn = DapperHelper.GetConnString())
            {
                string sql = @"select f.id,f.foodnumber,f.foodname,f.storenumber,f.createtime,f.foodsprice,f.filename,t.typename,s.storename,f.foodsummary from Food f left join foodtype t on f.foodtypeid=t.id join store s on f.storenumber=s.storenumber where s.id=:id ";
                var foodList = conn.Query<Food>(sql, new { id=id});
                return foodList.ToList<Food>();
            }
        }
